...
export default async function StudentDetailPage({
  ...
  const enrollmentRows = await db
    ...
    .where(
      and(
        ...
        eq(enrollments.schoolId, session.user.schoolId!),
      ),
    )
  ...
  const activeEnrollment =
    enrollmentRows.find((e) => e.status === "active") ?? null;
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ID: {student.studentIdNumber} · Enrolled{" "}
            {new Date(student.enrollmentDate).toLocaleDateString("en-GH", {
              year: "numeric",
              month: "long",
            })}
          </p>
