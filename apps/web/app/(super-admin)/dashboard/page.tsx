  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard title="Active Schools" value={data?.activeSchools ?? 0} icon={ShieldCheck} />
        <StatCard title="Total Users" value={data?.totalUsers ?? 0} icon={Users} />
        <StatCard title="New Signups (30d)" value={data?.newSignupsLast30Days ?? 0} icon={UserPlus} />
          ...
          <Card>
            ...
            <CardContent>
              <ul className="space-y-3">
                {data.recentActivity.map((a: { id: string; action: string; tableName: string; createdAt: string }) => (
                  <li key={a.id} className="flex items-start gap-3 text-sm">
                    <Activity className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                    <div className="flex-1">
                      <p className="font-medium">{a.action}</p>
                      <p className="text-xs text-muted-foreground">
                        {a.tableName} · {new Date(a.createdAt).toLocaleString('en-GH')}
                      </p>
                    </div>
                  </li>
                ))}
