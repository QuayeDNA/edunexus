...
export const POST = routeHandler(async (request: NextRequest) => {
  ...
  const termData = [
    {
      ...
      name: "First Term",
      startDate: new Date(year, 8, 1),
      endDate: new Date(year, 11, 20),
    },
    {
      termNumber: "2",
      name: "Second Term",
      startDate: new Date(year + 1, 0, 7),
      endDate: new Date(year + 1, 3, 11),
    },
    {
      termNumber: "3",
      name: "Third Term",
      startDate: new Date(year + 1, 4, 5),
      endDate: new Date(year + 1, 6, 31),
    },
  ];
  await db
  ...
  await db
    .insert(gradeLevels)
    .values(gradeData.map((g) => ({ schoolId: school.id, ...g })));
