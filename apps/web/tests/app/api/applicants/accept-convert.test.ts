...
describe("POST /api/applicants/[id]/accept — full conversion", () => {
  it("converts applicant to student, enrollment, guardian atomically", async () => {
    ...
    mockDb.limit.mockResolvedValueOnce([
      { id: schoolId, code: "AABS001", name: "Test School" },
    ]);
    mockDb.limit.mockResolvedValueOnce([
      { id: academicYearId, isCurrent: true, schoolId },
    ]);
    ...
    txReturning
      ...
      .mockResolvedValueOnce([
        { id: "student-profile-1", email: "aabs20260001@edunexus.com" },
      ])
      .mockResolvedValueOnce([
        { id: "parent-profile-1", email: "jane@example.com" },
      ])
      .mockResolvedValueOnce([
        { id: "app-1", status: "accepted", targetClassId: classId },
      ]);
    ...
    expect(body.data.credentials.student.email).toBe(
      "aabs20260001@edunexus.com",
    );
  ...
  it("rejects if applicant status is not under_review or waitlisted", async () => {
    mockDb.limit.mockResolvedValueOnce([
      makeApplicant({ status: "submitted" }),
    ]);
