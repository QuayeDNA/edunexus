...
function createDbMock() {
  ...
  const proxy = new Proxy(target, {
    get(_t, prop) {
      ...
      if (typeof prop === 'string' && ['select', 'from', 'where', 'orderBy', 'limit'].includes(prop)) {
        return () => proxy;
      }
      return undefined;
    },
...
describe('POST /api/students/import', () => {
  ...
  it('preview returns headers and suggested mapping', async () => {
    ...
    const res = await POST(new Request('http://localhost:3000/api/students/import/preview', {
      ...
      body: JSON.stringify({ csv: 'firstName,lastName,gender\nJohn,Doe,male' }),
    }) as any);
  ...
  it('execute imports valid rows and reports failed rows', async () => {
    mockDbState = {
      ...
      results: [
        ...
        [{ id: 'class-1', code: 'SS1A', name: 'SS 1A', gradeLevelId: 'g-1', schoolId: 'school-1' }],
