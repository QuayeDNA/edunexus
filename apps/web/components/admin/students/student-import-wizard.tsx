        ...
        <CardContent className="pt-6 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Headers: firstName, lastName, gender, dateOfBirth, classCode,
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Map CSV columns to student fields. Fields without a mapping will be
            skipped.
          </p>
          {headers.map((header) => (
            <div key={header} className="flex items-center gap-3">
              <Label className="w-40 shrink-0 text-sm font-mono">
                {header}
              </Label>
              <Select
                value={mapping[header] ?? ""}
                onValueChange={(v) =>
                  setMapping((p) => ({ ...p, [header]: (v as string) ?? "" }))
                }
                items={KNOWN_FIELDS.map((f) => ({
                  value: f,
        ...
        <CardContent className="space-y-4">
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {validationResult?.rows
                      .filter((r: ValidationRow) => !r.valid)
                      .map((r) => (
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={
                importMutation.isPending || (validationResult?.valid ?? 0) === 0
              }
            >
              {importMutation.isPending
                ? "Importing..."
                : `Import ${validationResult?.valid ?? 0} Valid Students`}
            </Button>
          </div>
        </CardContent>
      </Card>
