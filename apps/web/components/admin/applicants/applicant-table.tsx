...
export function ApplicantTable({ gradeLevels }: ApplicantTableProps) {
  ...
  const queryKey = [
    "applicants",
    { status, gradeLevelId, search: committedSearch, page },
  ] as const;
  const applicantsQuery = useQuery({
    queryKey,
    queryFn: () =>
      fetchApplicants({ status, gradeLevelId, search: committedSearch, page }),
  });
  ...
  useEffect(() => {
    setPage(1);
  }, [status, gradeLevelId, committedSearch]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="w-48">
          <Select
            value={gradeLevelId}
            onValueChange={(value) => setGradeLevelId(value as string)}
