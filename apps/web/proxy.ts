...
async function fetchTenantByHost(hostname: string): Promise<TenantInfo> {
  try {
    ...
    const res = await fetch(
      `${baseUrl}/api/internal/resolve-tenant?hostname=${encodeURIComponent(hostname)}`,
      {
...
const proxyHandler = auth(async function proxy(
