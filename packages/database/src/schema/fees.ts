import {
  pgTable, uuid, text, timestamp, varchar, numeric, date, boolean, index,
...
export const feeCategories = pgTable('fee_categories', {
  ...
  index('idx_fee_categories_school_id').on(table.schoolId),
]);
...
export const feeSchedules = pgTable('fee_schedules', {
  ...
  feeCategoryId: uuid('fee_category_id').notNull().references(() => feeCategories.id),
...
export const studentFees = pgTable('student_fees', {
  ...
  feeScheduleId: uuid('fee_schedule_id').notNull().references(() => feeSchedules.id),
  amount: numeric('amount', { precision: 12, scale: 2 }).notNull(),
