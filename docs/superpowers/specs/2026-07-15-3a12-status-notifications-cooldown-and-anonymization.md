...
if (existing) {
  const cooldownEnd = new Date(existing.createdAt.getTime() + 180 * 24 * 60 * 60 * 1000);
  if (cooldownEnd > new Date()) {
    return apiError(409, `You may re-apply after ${cooldownEnd.toLocaleDateString('en-GH', { year: 'numeric', month: 'long', day: 'numeric' })}`);
  }
...
```typescript
export async function anonymizeApplicant(client: typeof db, applicantId: string): Promise<void>
```
