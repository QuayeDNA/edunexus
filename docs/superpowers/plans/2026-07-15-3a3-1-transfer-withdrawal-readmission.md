...
describe("updateEnrollmentStatus", () => {
  ...
  it("rejects invalid transition (graduated → withdrawn)", async () => {
    ...
    await expect(
      updateEnrollmentStatus({ enrollmentId: "e1", newStatus: "withdrawn" }),
    ).rejects.toThrow(
...
const { generateTransferCertificate } =
  await import("@/services/transfer-certificate");
...
describe("POST /api/enrollments/[id]/transfer", () => {
  it("returns 200 with certificate info on successful transfer", async () => {
    mockLifecycle.mockResolvedValue({
      ...
      endDate: "2026-07-15",
      transferReason: "Moved to Accra Academy",
      transferSchoolName: "Accra Academy",
    });
    ...
    const res = await POST(
      new Request("http://localhost:3000/api/enrollments/e1/transfer", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reason: "Moved to Accra Academy",
          targetSchoolName: "Accra Academy",
        }),
      }) as any,
      { params: { id: "e1" } } as any,
    );
...
describe("POST /api/students/[id]/re-admit", () => {
  ...
  it("re-admits a withdrawn student", async () => {
    mockDb.limit
      ...
      .mockResolvedValueOnce([
        { id: "y1", schoolId: "school-1", isCurrent: true },
      ]);
    mockDb.returning.mockResolvedValue([
      { id: "e1", status: "active", classId: "c1", academicYearId: "y1" },
    ]);

    const { POST } = await import("@/app/api/students/[id]/re-admit/route");
    const res = await POST(
      new Request("http://localhost:3000/api/students/s1/re-admit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  it("rejects re-admission for active student", async () => {
    mockDb.limit.mockResolvedValueOnce([
      { id: "s1", status: "active", schoolId: "school-1" },
    ]);
...
describe("GET /api/students/inactive", () => {
  ...
  it("returns inactive students", async () => {
    ...
    const res = await GET(
      new Request("http://localhost:3000/api/students/inactive") as any,
    );
...
export async function POST(
  ...
  if (student.status === "active")
    return apiError(422, "Student is already active");
